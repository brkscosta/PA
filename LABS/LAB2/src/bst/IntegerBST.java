...
public class IntegerBST implements IntegerBinarySearchTree {
    ...
    private int sum(TreeNode treeRoot) {
        ...
        int value = 0;
        value += treeRoot.element;

        if (treeRoot.hasLeft()) {
            value += sum(treeRoot.left);
        }

        if (treeRoot.hasRight()) {
            value += sum(treeRoot.right);
        }
    ...
    private int sumInternals(TreeNode treeRoot){
        int count = 0;
        
        if(treeRoot.hasLeft() || treeRoot.hasRight()) {
            if(treeRoot != this.root)
                count += treeRoot.element;
            
            if(treeRoot.hasLeft())
                count += sumInternals(treeRoot.left);
            
            if(treeRoot.hasRight())
                count += sumInternals(treeRoot.right);
        }
    ...
    public int countGreaterThan(Integer value) throws EmptyContainerException {
        if(this.root == null)
            throw new EmptyContainerException();
        return countGreaterThan(this.root, value);
        
    }

    private int countGreaterThan(TreeNode treeRoot, Integer value) {
        
        int count = 0;
        
        if(treeRoot.element > value)
            count += 1;
        
        if(treeRoot.hasLeft())
            count += countGreaterThan(treeRoot.left, value);
        
        if(treeRoot.hasRight())
            count += countGreaterThan(treeRoot.right, value);
    ...
    public Set<Integer> greaterThan(Integer value) {
        ...
        greatherThan(this.root, value, set);
        
        return set;
    }

    private void greatherThan(TreeNode treeRoot, Integer value, Set<Integer> set) {
        
        if(treeRoot.element > value)
            set.add(treeRoot.element);
        
        if(treeRoot.hasLeft())
            greatherThan(treeRoot.left, value, set);
        if (treeRoot.hasRight()) 
            greatherThan(treeRoot.right, value, set);
    ...
    private Iterable<Integer> breathTransversal(TreeNode treeRoot) {
    ...
    public Iterable<Integer> breadthOrder() {
        return breathTransversal(this.root);
    }
